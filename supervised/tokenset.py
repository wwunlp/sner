...
class Token:
    ...
    def key(self):
        return ((self.token + str(self.left_context) + str(self.right_context)), self.annotation.value)
    ...
    def __hash__(self):
        return hash(self.token + str(self.left_context) + str(self.right_context) + str(self.annotation.value))
...
#it will enforce 'Set' style behavior on the tokens in that no two identical token objects will be allowed
class TokenSet:
    tokens = dict()
    ...
    def addToken(self, token):
        if(type(token) is not Token):
            tb = sys.exc_info()[2]
            raise TypeError("Tried to push something other than a token into a TokenSet").with_traceback(tb)
    ...
    def extend(self, setOfTokens):
        if(type(setOfTokens) is not TokenSet):
            tb = sys.exc_info()[2]
            raise TypeError("Attempted to add contents of TokenSet to another TokenSet, was given something other than a TokenSet to add").with_traceback(tb)
