...
def getPersonalNames(csvFile):
    """
    Find names in given csv file and returns them in a dictionary with their counts
    Assumes that the 5th value on each line is the actual name and
    that the 9th value is an identifier for the type with "PN" indicating a Personal name
    """
...
def getKgrams(names, k):
    """
    names is assumed to be a dictionary with keys representing names and values their occurences
    k is the max order of grams you wish to retrieve
    -1 will return a list containing a dictionary of monograms
    -2 will return a list with a dict of monograms followed by a dict of bigrams
